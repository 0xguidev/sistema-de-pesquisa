...
describe('Fetch Questions By Survey Id', () => {
  ...
  it('should be able to fetch questions by survey id', async () => {
    ...
    if (result.isRight()) {
      expect(result.value.question[0].id.toString()).toBe(
        question1.id.toString(),
      )
      expect(result.value.question[0].questionTitle).toBe('Question 1')
      expect(result.value.question[0].questionNum).toBe(1)
      expect(result.value.question[1].id.toString()).toBe(
        question2.id.toString(),
      )
      expect(result.value.question[1].questionTitle).toBe('Question 2')
      expect(result.value.question[1].questionNum).toBe(2)
    }
  })
